...
%
%   Figures:
%       fig_throughput_noninvariance.pdf
...
%   cycle table. This can later be replaced by cdfdSignedCycleSummaries.m.
    ...
    %% Figure 0: throughput is not representation-invariant

    fig0 = makeThroughputNoninvarianceFigure(Wtri, visibility);

    exported = exportExampleFigure(fig0, outputDir, ...
        'fig_throughput_noninvariance', opts);

    results.ExportedFiles = [results.ExportedFiles; exported(:)];

    results.Figures.ThroughputNoninvariance = fig0;
...
function fig = makeThroughputNoninvarianceFigure(Ctri, visibility)
% makeThroughputNoninvarianceFigure
% Illustrate that completed circular throughput is not invariant across
% cycle representations of the same circulation.

    fig = figure( ...
        'Color', 'w', ...
        'Name', 'Throughput non-invariance under cycle representation', ...
        'Visible', visibility);

    fig.Position = [100 100 1450 560];
    ...
    p1.EdgeLabel = repmat("1", numedges(G), 1);

    title(ax1, '(a) Same circulation $C$', ...
        'Interpreter', 'latex');

    text(ax1, 0.5, -0.23, ...
        {'Unit bidirected triangle', '$V_C=\sum_e c_e=6$'}, ...
    ...
    p2.EdgeLabel = repmat("", numedges(G), 1);

    title(ax2, '(b) Three two-cycle representation', ...
        'Interpreter', 'latex');

    text(ax2, 0.5, -0.20, ...
        { ...
        '$\lambda_{(1,2)}=\lambda_{(1,3)}=\lambda_{(2,3)}=1$', ...
        '$T_C=1+1+1=3$', ...
        '$V_C=3\times 2=6$' ...
        }, ...
        'HorizontalAlignment', 'center', ...
    ...
    p3.EdgeLabel = repmat("", numedges(G), 1);

    title(ax3, '(c) Two three-cycle representation', ...
        'Interpreter', 'latex');

    text(ax3, 0.5, -0.20, ...
        { ...
        '$\lambda_{(1,2,3)}=\lambda_{(1,3,2)}=1$', ...
        '$T_C=1+1=2$', ...
        '$V_C=2\times 3=6$' ...
        }, ...
        'HorizontalAlignment', 'center', ...
    ...
    annotation(fig, 'textbox', [0.24, 0.01, 0.52, 0.055], ...
        'String', ['The same circulation has invariant edge-volume ', ...
                   'V_C=6, but different completed throughput T_C under ', ...
                   'different cycle representations.'], ...
        'EdgeColor', 'none', ...
        'HorizontalAlignment', 'center', ...
        'Interpreter', 'none');
end
